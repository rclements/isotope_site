  ...
  describe "A logged-in user" do
    ...
    describe "posting to #create" do
      describe "with valid parameters" do
        before(:each) do
          @user = User.make
          post :create, { :user_id => @user.id, :file_attachment => { :attachment_file_file_name => "aasdgsdgr" } }
        end
      ...
      describe "with invalid parameters" do
        before(:each) do
          @user = User.make
          post :create, { :user_id => @user.id, :file_attachment => {:attachment_file_file_name => nil } }
        end
        it { response.should render_template("file_attachments/new") }
      end
    end
